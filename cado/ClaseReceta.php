<?php
   ...
   class Recetas{
	function ListarRe($idpac){
	  $ocado=new cado();
	  $sql="select top 50 fec_crea,subtotal,descuento,total,estado,
	        medico,id,examen,id_paciente,id_medico,isnull(acuenta,0.00),
			total-isnull(acuenta,0.00)
	        from receta r where id_paciente=$idpac order by fec_crea desc";
	...
	function ListarDetalle($id){
	  $ocado=new cado();
	  $sql="select id,nombre,precio_part from examen where id in($id)";
	  $ejecutar=$ocado->ejecutar($sql);
	  return $ejecutar;
	}
	function MaxOrden($user){
       $ocado=new cado();
	   $sql="select isnull(correlativo,0)correlativo,(select codigo_ingreso from caja_fondos where activo=1 and nom_user='$user')codigo_ingreso from serie where tipo_doc='OR' ";
		  $ejecutar=$ocado->ejecutar($sql);
		  return $ejecutar;
	 }
	...
	function RegistrarReceta($subtotal,$descuento,$total,$id_pac,$id_med,$medico,$porcentaje,$monto_med,$user,$idconv,$carrito,$emp,$domicilio,$fec_dom,$lugar,$tipo_pago,$turno,$fecemision,$nrocarta,$nrosolicitud,$montocarta){
		 ...
		 // $cn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		  $cn->beginTransaction();
		  $sql_max_orden="select isnull(correlativo,0),
		  isnull((select rtrim(ltrim(codigo_ingreso)) from caja_fondos where activo=1 and nom_user='$user'),0)
		 ...
		 // if($sucursal==0){$codigo_ingreso=$datos[1];}else{$codigo_ingreso="0".date('dmY').'LIDER';}
		  ...
		  if(strlen($codigo_ingreso)>0){
		       ...
		       if($emp=='I'){$estado=0;}*/
			   $estado=0;
			   if($domicilio==1){$credito=0;}
			  // primero insertamos la cabecera
			   $sql="insert into receta(fec_crea,examen,subtotal,descuento,total,estado,id_paciente,id_medico,medico, porcentaje_med,monto_medico,codigo_ingreso,nro_orden,id_convenio,emp,credito,domicilio,fec_domicilio,lugar_dom,tipo_pago_dom,turno,procesos,whatsap,anulado,doc_emitido,ingreso_dinero,pago_medico,estado_dom,muestra) 	            values(getdate(),'$id_examen',$subtotal,$descuento,$total,$estado,$id_pac,$id_med,'$medico',$porcentaje,$monto_med,'$codigo_ingreso',
					replicate('0',8-len($nro_orden))+cast($nro_orden as varchar),$idconv,'P','$credito',$domicilio,'$fec_dom',
					'$lugar','$tipo_pago','$turno',0,0,0,0,0,0,0,0)";
			  //die($sql);exit;			  
			   ...
			   $receta_id= $cn->lastInsertId();
			     ...
			     for($i=0;$i<count($carrito);$i++){
				   ...
				   $sql_detalle="INSERT INTO receta_detalle(id_receta,id_examen,precio_part,precio_conv,precio_part_desc,cantidad,pago_paciente,pago_convenio,subtotal,informe_cargado,estado,toma_muestra,recepcion_proceso,finalizado)
				   VALUES ('$receta_id','$examen_id','$precio_part','$precio_conv','$precio_part_desc',1,'$pago_paciente','$pago_conv','$subtotal_detalle',0,0,0,0,0)";
					//die($sql_detalle);exit;
			  ...
			  /*if($credito==0){
			     $sql_cola="select max(nro_ticket) from cola_ticket where cast(fecha as date)=cast(getdate() as date)";
				  echo $sql_cola;exit;
		         $cmd_cola=$cn->prepare($sql_cola);$cmd_cola->execute();
		         $dato_cola=$cmd_cola->fetch();
		         $nroticket=$dato_cola[0]+1;
				 $sql_cola_insert="insert into cola_ticket (id_receta,id_paciente,fecha,nro_ticket) values($receta_id,$id_pac,getdate(),$nroticket)";
				  //die($sql_cola_insert);exit;
	...
	function RegistrarCuentaCorriente($cod,$orden,$tipo_doc,$nro_serie,$nro_documento,$total_venta,$fecha,$hora){
		  ...
		  values('$cod','$orden','$tipo_doc','$nro_serie','$nro_documento','$total_venta',getdate(),getdate(),'$total_venta',0);";
		  $cn->prepare($sql)->execute();
		  $cn->commit();
		  $cn=null;
		  $return=1;
	...
	function RegistrarCredito($cod_ingreso,$orden,$total_venta,$empresa,$chek,$credito){
		  try{
		  $ocado=new cado();
		  $cn=$ocado->conectar();
		  $cn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		  ...
		  if($chek==1){
		   ...
		   $sql_caja="insert into caja_fondos_detalle (codigo_ingreso,nro_orden,total_venta,tipo_pago,monto_efectivo,movimiento,
					  ...
					  values('$cod_ingreso','$orden','$total_venta','E','$total_venta','PAGO CREDITO',getdate(),getdate(),'P')";
		   //die($sql_caja);exit;
		   $cn->prepare($sql_caja)->execute();
		  }
		  $sql="update receta set credito=$credito,ingreso_dinero=$in where nro_orden='$orden'";
		  $cn->prepare($sql)->execute();
	...
	function ListarRecetaExamen($orden){
		  $ocado=new cado();
		  $sql="SELECT STRING_AGG((cast(id_examen as varchar)+'-'+cast(d.subtotal as varchar)),';') examen_precio 
	...
	function RegistrarFacturacionElectronica($cod_operacion,$cod_dom_fiscal,$tipo_documento,$tipodoc_cli,$doc_cliente,$nomcli,$dircliente,$moneda,$op_gravadas,$op_gratuitas,$op_inafecta,$op_exoneradas,$total_descuento,$total_igv,$total_isc,$total_otrtri,$total_otrcar,$total_glosa,$valor_venta,$importe_total,
$idcliente,$array_examen,$cod,$orden,$tipo_doc,$nro_serie,$nro_documento,$ruc,$razon,$dir,$total_venta,$tipo_pago, 
		 $monto_efectivo,$monto_tarjeta,$monto_pagado,$vuelto,$visa,$mastercard,$otros,$emp,$nrodeposito,$mon_dep,$mon_yape){
		try{  
		  $ocado=new cado();
		  $cn=$ocado->conectar();
		  $cn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		  $cn->beginTransaction();
          if($tipo_documento=='01'){$td='FA';}
          if($tipo_documento=='03'){$td='BV';}
          if($tipo_documento=='07'){$td='NC';}
          if($tipo_documento=='TK'){$td='TK';}
          $sql_doc="select serie,correlativo+1 corre,REPLICATE('0',(8-LEN(correlativo+1))) + cast(correlativo+1 as varchar(8)) numdoc
                   from caja_fondos c inner join caja ca on c.id_caja=ca.id
                   inner join caja_series s on ca.id=s.id_caja
                   inner join serie se on s.id_serie=se.id
                   where codigo_ingreso='$cod' and tipo_doc='$td' ";
          //die($sql_doc);
		  $cmd= $cn->prepare($sql_doc);
		  $cmd->execute(); 
		  $data=$cmd->fetch();
		  $serie=$data[0];$correlativo=$data[1];$numdoc=$data[2];
          
          if($serie==''){return 'NO';exit;}
		  //$sql_validar="select count(*) from doc_electronicos where serie";			
		  $sql="insert into doc_electronicos(cod_operacion,fecha_emision,hora_emision,cod_dom_fiscal,tipo_documento,serie,correlativo,tipodoc_cli,
doc_cliente,nomcli,dircliente,moneda,op_gravadas,op_gratuitas,op_inafecta,op_exoneradas,total_descuento,total_igv,total_isc,
total_otrtri,total_otrcar,total_glosa,valor_venta,importe_total,created_at,id_cliente,grupal,emp,firma_digital,otros,envio_sunat,
cod_res_sunat,envio,xml_generado,pdf_generado,resp_cdr,estado,titulo_gratuito)  	  values('$cod_operacion',getdate(),getdate(),'$cod_dom_fiscal','$tipo_documento','$serie','$numdoc','$tipodoc_cli','$doc_cliente','$nomcli','$dircliente','$moneda','$op_gravadas','$op_gratuitas','$op_inafecta','$op_exoneradas','$total_descuento','$total_igv','$total_isc','$total_otrtri','$total_otrcar','$total_glosa','$valor_venta','$importe_total',getdate(),$idcliente,0,'P','','',0,'',0,'',0,0,0,0);";
	...
	VALUES ('$doc_electronico_id', $incremental, 2, '10', 'ZZ', 1, '$descripcion_item',replicate('0',5-len($cod_interno))+cast($cod_interno as varchar),'$valuni','$valigv', '$preuni', '$valven', getdate())";
			//die($sql_detalle);exit;
             $cn->prepare($sql_detalle)->execute();
		  ...
		  if($vuelto==''){$vuelto=0;}
		  if($vuelto==''){$vuelto=0;}
		  $sql_caja_fondos="insert into caja_fondos_detalle(codigo_ingreso,nro_orden,tipo_doc,nro_serie,nro_documento,ruc,razon,direccion,total_venta,tipo_pago,  monto_efectivo,monto_tarjeta,monto_pagado,vuelto,visa,mastercard,otros,fec_emision,hora_emision,estado,movimiento,empresa,nrodeposito,monto_egreso,total_sin_retencion,monto_retencion,monto_deposito,monto_yape)  
		  values('$cod','$orden','$tipo_doc','$serie','$numdoc','$ruc','$razon','$dir','$total_venta','$tipo_pago', 
 '$monto_efectivo','$monto_tarjeta','$monto_pagado','$vuelto','$visa','$mastercard','$otros',getdate(),getdate(),0,'VENTA','P','$nrodeposito',0,0,0,'$mon_dep','$mon_yape');";
			
          //die($sql_caja_fondos);exit;
	...
	function Eliminar($nro_orden,$user){
	  try{
		  ...
		  $cn=$ocado->conectar();
		  $cn->beginTransaction();
		  $sql="update receta set anulado=1,user_anula='$user',fecha_anula=getdate() where nro_orden = '$nro_orden' ";
		  $cn->prepare($sql)->execute();
	...
	/*function BuscarOrden($orden){
		...
		(p.ape_pat+' '+p.ape_mat+' '+isnull(p.preNombres,''))as paciente,r.id_medico,r.anulado,r.examen_precio,p.dni,p.id idpac, r.doc_emitido,r.emp,r.credito,p.telefono
		      from receta r inner join paciente p on r.id_paciente=p.id
			   where cast(r.nro_orden as int) =  cast($orden as int)";
		$ejecutar=$ocado->ejecutar($sql);
		return $ejecutar;
	}*/
	function BuscarOrden($orden){
		$ocado=new cado();
		$sql="select r.id,r.nro_orden,r.examen,r.porcentaje_med,r.monto_medico,r.subtotal,r.descuento,r.total,r.estado,r.id_convenio,
		(p.ape_pat+' '+p.ape_mat+' '+isnull(p.preNombres,''))as paciente,r.id_medico,r.anulado,r.examen_precio,p.dni,p.id idpac, r.doc_emitido,case when (select count(*) from receta_detalle d inner join examen exa on d.id_examen=exa.id and cod_tipo='03' where d.id_receta=r.id)>0 then 'I' else 'P' end emp,r.credito,p.telefono
		      from receta r inner join paciente p on r.id_paciente=p.id
			   where cast(r.nro_orden as int) =  cast($orden as int)";
		$ejecutar=$ocado->ejecutar($sql);
		return $ejecutar;
	...
	function ListarDetalleOrdenNuevo($idreceta){
		$ocado=new cado();
		$sql="select e.nombre,d.subtotal,m.nombre as muestra,g.nombre gru,tiempo_resultado
	...
	function InsertarResultado($orden){
		  ...
		  $sql=" insert into resultado(id_receta,id_examen,examen,id_caracteristica,caracteristica,titulo,resultado,unidad,rango,metodo)  
		         select t.id_receta,t.id_exa,t.nombre,c.id,ISNULL(c.nombre,'') nombre,c.titulo,c.resultado,c.unidad,c.referencia,c.metodo
                 from (
	...
	function LisRes(){
		$ocado=new cado();
		$sql="select r.id,fec_crea,(select (ape_pat +' '+ape_mat+' '+preNombres) nom_pac from paciente p where p.id=r.id_paciente )paciente,estado,id_paciente,
	...
	function LisRes1($bus,$p){
		if($p==0){
		  if($bus==''){$where="";}else{$where=" and (trim(ape_pat)+' '+trim(ape_mat)+' '+trim(preNombres)) like '%$bus%'";}
		}
		if($p==1){
		  if($bus==''){$where=" and r.estado=1 ";}else{$where=" and r.estado=1 and (trim(ape_pat)+' '+trim(ape_mat)+' '+trim(preNombres)) like '%$bus%'";}
		}
		
		$ocado=new cado();
		$sql="select top 200 r.id,nro_orden,r.fec_crea,
		(isnull(ape_pat,'')+' '+isnull(ape_mat,'')+' '+isnull(preNombres,''))paciente,r.estado,r.id_paciente,
			...
			(select count(*) from resultado re inner join examen e on re.id_examen=e.id
			  ...
			  FORMAT(fecha_entrega_dom,'dd/MM/yyyy hh:mm ss') fec_ent_dom,
			 case when domicilio=1 then FORMAT(fec_domicilio,'dd/MM/yyyy') else '' end fec_dom ,fec_domicilio,lugar_dom,tipoentregadom 
	...
	/*function LisResOrden($bus,$p,$tipoorden){
		...
		$ocado=new cado();
		$sql="select top 200 r.id,nro_orden,r.fec_crea,
		(isnull(ape_pat,'')+' '+isnull(ape_mat,'')+' '+isnull(preNombres,''))paciente,r.estado,r.id_paciente,m.nombre MEDICO,' ' examen,user_finaliza,r.user_anula,r.anulado,conv.ruc ruc,
			(select count(*) from resultado re inner join examen e on re.id_examen=e.id
		      where re.id_receta=r.id and  resul_externo=0) can,p.telefono,domicilio,obs_dom,
			  FORMAT(fecha_entrega_dom,'dd/MM/yyyy hh:mm ss') fec_ent_dom,
			 case when domicilio=1 then convert(varchar(12),fec_domicilio,103) else '' end fec_dom,fec_domicilio,lugar_dom,tipoentregadom,
			 whatsap from receta r left join medico m on r.id_medico=m.id
			              inner join paciente p on r.id_paciente=p.id
						  inner join convenio conv on r.id_convenio=conv.id
			where r.estado>0 and r.emp='P' and (trim(ape_pat)+' '+trim(ape_mat)+' '+trim(preNombres)) like '%$bus%'  order by $ordenar";
		$ejecutar=$ocado->ejecutar($sql);
		return $ejecutar;
	}*/
	function LisResOrden($bus,$p,$tipoorden){
		if($tipoorden==1){$ordenar=" r.id desc ";}
		if($tipoorden==2){$ordenar=" r.fecha_entrega_dom desc, r.estado asc";}
		$ocado=new cado();
		$sql="select top 200 r.id,nro_orden,r.fec_crea,
		(isnull(ape_pat,'')+' '+isnull(ape_mat,'')+' '+isnull(preNombres,''))paciente,r.estado,r.id_paciente,m.nombre MEDICO,' ' examen,user_finaliza,r.user_anula,r.anulado,conv.ruc ruc,
			(select count(*) from resultado re inner join examen e on re.id_examen=e.id
			  ...
			  FORMAT(fecha_entrega_dom,'dd/MM/yyyy hh:mm ss') fec_ent_dom,
			 case when domicilio=1 then convert(varchar(12),fec_domicilio,103) else '' end fec_dom,fec_domicilio,lugar_dom,tipoentregadom,
			 whatsap,case when id_convenio=8 then 'PARTICULAR' ELSE 'CONVENIO' END tipopac
             from receta r left join medico m on r.id_medico=m.id
			              inner join paciente p on r.id_paciente=p.id
						  inner join convenio conv on r.id_convenio=conv.id
			where r.estado>0 and r.emp='P' and (trim(ape_pat)+' '+trim(ape_mat)+' '+trim(preNombres)) like '%$bus%' and	(select count(*)  from receta re inner join  receta_detalle d on re.id=d.id_receta
			inner join examen exa on d.id_examen=exa.id and cod_tipo='02' where re.id=r.id
group by re.id ) > 0
			 order by $ordenar";
		$ejecutar=$ocado->ejecutar($sql);
		return $ejecutar;
	...
	function LisResOrdenToma($bus,$p,$tipoorden){
		...
		if($tipoorden==2){$ordenar=" r.fecha_entrega_dom desc, r.estado asc ";}
		$ocado=new cado();
		$sql="select top 50 r.id,nro_orden,r.fec_crea,
		(isnull(ape_pat,'')+' '+isnull(ape_mat,'')+' '+isnull(preNombres,''))paciente,r.estado,r.id_paciente,
		    m.nombre,' ' examen,user_finaliza,r.user_anula,r.anulado,conv.ruc ruc,
			(select count(*) from resultado re inner join examen e on re.id_examen=e.id
		      where re.id_receta=r.id and  resul_externo=0) can,p.telefono,domicilio,obs_dom,
			  FORMAT(fecha_entrega_dom,'dd/MM/yyyy hh:mm ss') fec_ent_dom,
		case when domicilio=1 then  convert(varchar(12),fec_domicilio,103) else '' end fec_dom ,fec_domicilio,lugar_dom,tipoentregadom,muestra,replace(convert(varchar,fecha_entrega_dom,20),' ','T')fec_ent_dom2
	        from receta r left join medico m on r.id_medico=m.id
			              inner join paciente p on r.id_paciente=p.id
						  inner join convenio conv on r.id_convenio=conv.id
			where r.estado=1 and r.anulado=0  and (trim(ape_pat)+' '+trim(ape_mat)+' '+trim(preNombres)) like '%$bus%' and muestra+procesos<2 and cast(r.fec_crea as date)>=cast('2021-04-23' as date)  order by $ordenar";
		$ejecutar=$ocado->ejecutar($sql);
		return $ejecutar;
	}
	function ModificarResOrdenToma($id,$fechaEntrega,$envio){
		  $ocado=new cado();
		  $sql="update receta 
                  set tipoentregadom='$envio',fecha_entrega_dom=cast(replace('$fechaEntrega','T',' ') as datetime)
                where id=$id";
	...
	/*function LisInforme($bus){
		$ocado=new cado();
		$sql="select top 50 r.id,nro_orden,r.fec_crea,
		(isnull(ape_pat,'')+' '+isnull(ape_mat,'')+' '+isnull(preNombres,''))paciente
		,r.estado,r.id_paciente,
		    m.nombre,(select nombre from examen e where cast(e.id as varchar)=r.examen )examen,user_finaliza,r.user_anula,r.anulado
	        from receta r left join medico m on r.id_medico=m.id
			              inner join paciente p on r.id_paciente=p.id
			where r.estado>0 and r.emp='I' and (trim(ape_pat)+' '+trim(ape_mat)+' '+trim(preNombres)) like '%$bus%' 
	...
	function LisInforme($bus){
		...
		(isnull(ape_pat,'')+' '+isnull(ape_mat,'')+' '+isnull(preNombres,''))paciente
		,r.estado,r.id_paciente,
		    m.nombre,user_finaliza,r.user_anula,r.anulado,count(d.id) can
	        from receta r left join medico m on r.id_medico=m.id
			              inner join paciente p on r.id_paciente=p.id
						  inner join receta_detalle d on r.id=d.id_receta
						  inner join examen exa on d.id_examen=exa.id and exa.cod_tipo='03'
			where r.estado>0  and (trim(ape_pat)+' '+trim(ape_mat)+' '+trim(preNombres)) like '%$bus%' 
			group by r.id,nro_orden,r.fec_crea,ape_pat,ape_mat,preNombres,r.estado,r.id_paciente,m.nombre,user_finaliza,r.user_anula,r.anulado
			  order by r.fec_crea desc";
		$ejecutar=$ocado->ejecutar($sql);
		return $ejecutar;
	}
	function LisReporte($pac,$inicio,$fin){
		if($pac==""){$where="";}else {$where=" and (trim(ape_pat) +' '+ trim(ape_mat)+' ' + trim(preNombres)) like '%$pac%'";}
		$ocado=new cado();
		$sql="select r.id,r.fec_crea,p.nom_pac,r.estado,p.id,
		    (select nombre from medico m where m.id=r.id_medico )medico,muestra,r.total,isnull(acuenta,0.00),total-isnull(acuenta,0.00)
	        ,(select nombre from examen e where cast(e.id as varchar)=r.examen )examen
			from receta r inner join paciente p on r.id_paciente=p.id
			 where r.estado>0 and cast(r.fec_crea as date)>=cast('2021-04-23' as date) and cast(r.fec_crea as date)<=cast('2021-04-23' as date)
			 $where
    ...
    function LisPdfResGrupalWp($id){
	   ...
	   from resultado r inner join examen e on r.id_examen=e.id 
		                inner join grupo g on e.id_grupo=g.id
		                inner join muestra m on e.muestra=m.id
		      where id_receta=$id and e.especial=0
			   order by g.nro_orden asc,e.nombre asc,r.id asc";
		$ejecutar=$ocado->ejecutar($sql);
		return $ejecutar;
	}
	...
	function LisPdfResAntigeno($id){
	   ...
	   from resultado r inner join examen e on r.id_examen=e.id 
		                inner join grupo g on e.id_grupo=g.id
		                inner join muestra m on e.muestra=m.id
			  where id_receta=$id and r.id_examen in( '641')
		order by g.nro_orden asc,e.nombre asc,r.id asc";
		$ejecutar=$ocado->ejecutar($sql);
		return $ejecutar;
	}
	...
	function VerHistorico($idreceta,$arreglo){
		$ocado=new cado();
		$sql="
		select rec.id,rec.id_paciente,resul.id_examen,e.nombre,(select count(*) from resultado rst where rst.id_receta=resul.id_receta and rst.id_examen=resul.id_examen and LEN(firma)>5) cant 
	         from  receta rec inner join resultado resul on rec.id=resul.id_receta
	                   inner join examen e on resul.id_examen=e.id
		       where rec.id=$idreceta and e.historico=1 and resul.id_examen in($arreglo) group by rec.id,rec.id_paciente,resul.id_examen,e.nombre,resul.id_receta";
		$ejecutar=$ocado->ejecutar($sql);
		return $ejecutar;
	}
	...
	function LisPdfResHistorico($idreceta,$idpac,$idexamen){
		$ocado=new cado();
		$sql="select * from 
		(select top 10 year(fec_crea)anio,FORMAT(re.fec_crea, 'dd/MM/yyyy')fecha,r.resultado,
		replace(replace(replace(r.resultado,'<',''),'>',''),' ','')  resultado1,re.id
	   from receta re inner join resultado r on re.id=r.id_receta
		     where id_paciente=$idpac and id_receta<=$idreceta and r.id_examen=$idexamen and trim(resultado)<>'' and re.anulado=0
	   order by re.id desc ) as t order by id asc";
		$ejecutar=$ocado->ejecutar($sql);
		return $ejecutar;
	}
	...
	function LisCarRes1($id){
		$ocado=new cado();
		$sql="select r.*,m.nombre,e.resul_externo ext,e.especial esp,r.user_ingresa,case when r.examen<>e.nombre then 
		(e.nombre+' ('+r.examen+')') else e.nombre end nom,de.estado
		      from resultado r inner join examen e on r.id_examen=e.id 
		           inner join grupo g on e.id_grupo=g.id
		           inner join muestra m on e.muestra=m.id
	...
	function LisCarRes2($id){
		$ocado=new cado();
		$sql="select r.*,m.nombre,e.resul_externo ext,e.especial esp,r.user_ingresa,case when r.examen<>e.nombre then concat(e.nombre,' (',r.examen,')') else e.nombre end nom,0 estado
		      from resultado r inner join examen e on r.id_examen=e.id 
		           inner join grupo g on e.id_grupo=g.id
		           inner join muestra m on e.muestra=m.id
	...
	function LisCarResPaquete1($id){
		$ocado=new cado();
		$sql="select count(*) from receta r inner join receta_detalle re on r.id=re.id_receta inner join examen e on re.id_examen=e.id
		  where paquete=1 and r.id=$id";
		$ejecutar=$ocado->ejecutar($sql);
		return $ejecutar;
	}
	...
	function LisCarResInformes($id){
		$ocado=new cado();
		$sql="select r.id,r.id_examen,inf.id,e.nombre,case when LEN(rtrim(ltrim(r.titulo)))>0 then r.titulo else inf.titulo end titu ,
		      case when LEN(rtrim(ltrim(r.contenido)))>0 then r.contenido  else inf.contenido end conte,case when LEN(rtrim(ltrim(r.contenido)))>0 then 1 else 0 end informado
	...
	function LisCarResSecrecion($id){
		$ocado=new cado();
		$sql="select * from
                   (select *,case resultado when 'Sensible' then '1' 
							when 'Intermedio' then '2'
	...
	function GrabarRes($id,$resultado,$user){
		  $ocado=new cado();
		  $sql="update resultado set resultado='$resultado',user_ingresa='$user',fec_hora_ingresa=getdate() where id='$id'";
		  $ejecutar=$ocado->ejecutar($sql);
		  return $ejecutar;
	}
	/*function GrabarResUser($id,$user){
		  $ocado=new cado();
		  $sql="update resultado set user_ingresa='$user',fec_hora_ingresa=getdate() where id=$id ";
		  $ejecutar=$ocado->ejecutar($sql);
		  return $ejecutar;
	}*/
	...
	function ListarPaciente($idpac){
	  $ocado=new cado();
	  $sql="select id,(ape_pat+' '+ape_mat+' '+isnull(preNombres,''))pacien,dni,sexo,fec_nac,telefono,fec_crea,crea_user,
	        DATEDIFF(YEAR,fec_nac, cast(getdate() as date))edad
	...
	function ListarPaciente1($idreceta){
	  $ocado=new cado();
	  $sql="select p.id,(ape_pat+' '+ape_mat+' '+isnull(preNombres,''))pacien,dni,sexo,fec_nac,telefono,
	  case when domicilio=0 then r.fec_crea else r.fec_domicilio end fec_crea ,p.crea_user,
	        (cast(floor(cast(datediff(day, fec_nac, getdate()) as float)/365) as varchar)+','+
		cast(floor((cast(datediff(day, fec_nac, getdate()) as float)/365-(floor(cast(datediff(day, fec_nac, getdate()) as float)/365)))*12) as varchar)+','+
		cast(floor((((cast(datediff(day, fec_nac, getdate()) as float)/365-(floor(cast(datediff(day, fec_nac, getdate()) as float)/365)))*12)-
		floor((cast(datediff(day, fec_nac, getdate()) as float)/365-(floor(cast(datediff(day, fec_nac, getdate()) as float)/365)))*12))*(365/12)) AS varchar)) as edad         
            ,medico,nro_orden,telefono
	...
	/*function FinalizarRes($id,$user){
		$ocado=new cado();
		$sql="update receta set estado=2,user_finaliza='$user',fecha_finalizacion=now() where id=$id";
		$ejecutar=$ocado->ejecutar($sql);
		return $ejecutar;
	}*/
	function FinalizarRes($id,$user){
	  try{
		  ...
		  $cn=$ocado->conectar();
		  $cn->beginTransaction();
		  $sql="update receta set estado=2,user_finaliza='$user',fecha_finalizacion=getdate() where id=$id";
		  $cn->prepare($sql)->execute();
		  $sql_detalle="update receta_detalle set fecha_resultado=getdate(),finalizado=1 where id_receta=$id";
		  $cn->prepare($sql_detalle)->execute();
		  //die($sql_up);exit;
		  $cn->commit();
		  $cn=null;
	...
	function ver1($examen){
	  $ocado=new cado();
	  $sql="select STRING_AGG(nombre, ',')general
	        from examen where id in($examen) and estado=0 ";
	  $ejecutar=$ocado->ejecutar($sql);
	  return $ejecutar;
	}
...
where cen_dialisis='$cen_dia' and tipo_seguro='$conv' and estado=0
	...
	function ValidarAnular2($orden,$credito){
		  if($credito==0){
		   $sql="select r.id,case when tipo_doc in('TB','TF') then  (tipo_doc+' - '+nro_serie+nro_documento) 
		        else (nro_serie+nro_documento) end doc 
	...
	function IngresarMigracion($receta_id,$examen_id,$precio,$tipo){
	 ...
	 function FinDetalle($idreceta,$idexamen){
		  ...
		  $cn=$ocado->conectar();
		  $cn->beginTransaction();
		  $sql="update receta_detalle set estado=1,fecha_resultado=getdate(),finalizado=1 where id_receta=$idreceta and id_examen=$idexamen";
		  $cn->prepare($sql)->execute();
		  $cn->commit();
		  $cn=null;
		  $return=1;
	...
	function ListarDomicilios($ini){
	  $ocado=new cado();
	  $sql="select r.id,(ape_pat+' '+ape_mat+' '+preNombres)pac,p.dni,telefono,direccion,nro_orden,total,m.nombre,r.nro_orden,
	  ...
	  r.estado_dom,r.estado,r.obs_dom,r.fecha_entrega_dom,tipo_pago_dom,lugar_dom,tipoentregadom,turno
	   ...
	   where domicilio=1 and anulado=0 and estado_dom=0 and fec_domicilio='$ini'
	...
	function InsertarResultadoDom($orden,$obs,$fec,$tipopago,$idreceta,$te){
		  ...
		  $cn->prepare($sql)->execute();
		  $sql_act="update receta set estado=1,tipo_pago_dom='$tipopago',fecha_entrega_dom=cast(replace('$fec','T',' ') as datetime),obs_dom='$obs', estado_dom=1,
		  fec_toma_muestra=getdate() , tipoentregadom='$te',credito=0
		  where id=$idreceta;";
			  //die($sql_act);exit;
		  $cn->prepare($sql_act)->execute();
	...
	function DetalleMuestra($id){
	  $ocado=new cado();
	  $sql="select r.id,(ape_pat+' '+ape_mat+' '+preNombres)pac,p.dni,telefono,direccion,nro_orden,total,m.nombre,r.nro_orden,r.estado_dom,
	  r.estado,r.obs_dom
	   from receta r inner join paciente p on r.id_paciente=p.id 
	                 inner join medico m on r.id_medico=m.id
	   where r.id=$id"; 
	...
	function VerDetalleMuestra($idreceta){
	  $ocado=new cado();
	  $sql="select d.id,d.id_examen,e.nombre,toma_muestra,fecha_toma,recepcion_proceso,fecha_procesos,finalizado,fecha_resultado
	        from receta_detalle d inner join examen e on d.id_examen=e.id where d.id_receta=$idreceta and toma_muestra=0"; 
	  $ejecutar=$ocado->ejecutar($sql);
	  return $ejecutar;
	}
	function VerDetalleExamenes($idreceta){
	  $ocado=new cado();
	  $sql="select d.id,d.id_examen,e.nombre,toma_muestra,fecha_toma,recepcion_proceso,fecha_procesos,finalizado,fecha_resultado,condiciones,tiempo_resultado
	        from receta_detalle d inner join examen e on d.id_examen=e.id where d.id_receta=$idreceta"; 
	  $ejecutar=$ocado->ejecutar($sql);
	  return $ejecutar;
	}
	function VerDetalleProcesos($idreceta){
	  $ocado=new cado();
	  $sql="select d.id,d.id_examen,e.nombre,toma_muestra,fecha_toma,recepcion_proceso,fecha_procesos,finalizado,fecha_resultado
	        from receta_detalle d inner join examen e on d.id_examen=e.id where d.id_receta=$idreceta and recepcion_proceso=0"; 
	  $ejecutar=$ocado->ejecutar($sql);
	  return $ejecutar;
	}
	
	function GrabarToma($idreceta,$fec_ent,$tipo,$obs,$ArreDetalle){
	  try{
		  $ocado=new cado();
		  $cn=$ocado->conectar();
		  $cn->beginTransaction();
		  $sql_val="select count(*) from  receta_detalle where id_receta=$idreceta";
		  $cmd=$cn->prepare($sql_val);
		  $cmd->execute();
		  $col=$cmd->fetch();
		  $can=$col[0];
		  $can_arreglo=count($ArreDetalle);
		  $set='';
		  if($can==$can_arreglo){$set=",muestra=1 ";}
		  for($i=0;$i<count($ArreDetalle);$i++){
			 $sql="Update receta_detalle set toma_muestra=1,fecha_toma=getdate() where id=".$ArreDetalle[$i];
			 //die($sql);exit;
		     $cn->prepare($sql)->execute();
		  }
		  
		$sql_up="update receta set fecha_entrega_dom=cast(replace('$fec_ent','T',' ') as datetime),fec_toma_muestra=getdate(),tipoentregadom='$tipo',obs_dom='$obs' $set
		        where id=$idreceta";
		  //die($sql_up);exit;
		  $cn->prepare($sql_up)->execute();
	...
	function GrabarProcesos($idreceta,$ArreDetalle){
	  try{
		  $ocado=new cado();
		  $cn=$ocado->conectar();
		  $cn->beginTransaction();
		  ...
		  $can_arreglo=count($ArreDetalle);
		  $set='';
		  if($can==$can_arreglo){$set=",procesos=1 ";}
		  for($i=0;$i<count($ArreDetalle);$i++){
			 $sql="Update receta_detalle set recepcion_proceso=1,fecha_procesos=getdate() where id=".$ArreDetalle[$i];
			 //die($sql);exit;
		     $cn->prepare($sql)->execute();
		  }
		  //die('hola');exit;
		  $sql_up="update receta set fec_recep_procesos=getdate() $set where id=$idreceta";
		  $cn->prepare($sql_up)->execute();
		  //die($sql_up);exit;
		  $cn->commit();
		  $cn=null;
	 ...
	 function EnvioWhat($id){
		$ocado=new cado();
		$sql="update receta set whatsap=1 where id=$id";
		$ejecutar=$ocado->ejecutar($sql);
		return $ejecutar;
	 }
	 function AgregarExa($orden,$idexamen,$idconv,$precio){
		try{
		  $ocado=new cado();
		  $cn=$ocado->conectar();
		  $cn->beginTransaction();
		  $sql="select id from  receta where nro_orden=$orden";
		  $cmd=$cn->prepare($sql);
		  $cmd->execute();
		  $col=$cmd->fetch();
		  $idreceta=$col[0];
		  ...
		  $tipo=$datos_conv[0];$tarifario=$datos_conv[1];
		   ...
		   if($tipo=='PD'){$precio_part=0.00;$precio_conv=0.00;$precio_part_desc=$precio;$pago_paciente=$precio;$pago_conv=0.00;}
		   $subtotal_detalle=$pago_paciente+$pago_conv;
		  $sql_insert="INSERT INTO receta_detalle(id_receta,id_examen,precio_part,precio_conv,precio_part_desc,cantidad,pago_paciente,pago_convenio, subtotal) VALUES ('$idreceta','$idexamen','$precio_part','$precio_conv','$precio_part_desc',1,'$pago_paciente','$pago_conv','$subtotal_detalle')";
		  $cn->prepare($sql_insert)->execute();
		  
		  $sql_up="update receta set subtotal=subtotal+$subtotal_detalle, total=total+$subtotal_detalle where id=$idreceta";
		  $cn->prepare($sql_up)->execute();
	...
	function TicketActualizar(){
		$ocado=new cado();
		$sql="SELECT id,id_receta
		FROM ticket_pago WHERE cast(fec_genera_ticket as varchar)='2021-05-31'";
		$ejecutar=$ocado->ejecutar($sql);
		return $ejecutar;
	}
	function CalcularMontoMedico($idreceta){
		$ocado=new cado();
		$sql="SELECT sum(monto_medico) FROM receta where id in($idreceta) ";
		$ejecutar=$ocado->ejecutar($sql);
		return $ejecutar;
	}
	function ActualizarTick($id,$monto){
		$ocado=new cado();
		$sql="update ticket_pago set monto=$monto where id=$id";
		$ejecutar=$ocado->ejecutar($sql);
		return $ejecutar;
	}
    function VerNroCel($nro_orden){
		$ocado=new cado();
		$sql="select  p.telefono,p.ape_pat+' '+ape_mat+' '+preNombres pacien from receta r inner join paciente p on r.id_paciente=p.id where nro_orden='$nro_orden'";
		$ejecutar=$ocado->ejecutar($sql);
		return $ejecutar;
	}
    function ListaPacEnviosWhat(){
		$ocado=new cado();
		/*$sql="  select * from (
  select ROW_NUMBER() OVER(ORDER BY id desc) fila,telefono,preNombres,ape_pat,ape_mat
  from paciente_ant where len(telefono)=9 and tipo_ingreso='R' and telefono like '9%'  
  ) as t where fila>=1001 and fila<=1500";*/
        //$sql="select ";
		$ejecutar=$ocado->ejecutar($sql);
		return $ejecutar;
	}
